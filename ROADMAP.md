...
- Public `MattermostLiveSyncService` facade that emits live-sync lifecycle events with host-visible connection-state projection, reports non-cancellation backfill failures while keeping the lifecycle alive for later reconnect retry, runs connect/reconnect backfill with capped-by-default and opt-in all-joined-channel modes, applies WebSocket events into SwiftData, refreshes unread counts from `channel_viewed`/`post_unread` events where possible, refreshes per-user thread state from thread invalidation events where possible, and has a live CLI posted-event cache check.
...
- The SDK uses native `URLSession` and `URLSessionWebSocketTask` transports; live verification should diagnose server/proxy upgrade failures rather than relying on alternate process-based transports.
...
- `POST /users/login` returns the documented `Token` response header on the current server. The SDK also sends Mattermost's official web-client `X-Requested-With: XMLHttpRequest` header and accepts the `MMAUTHTOKEN` cookie in that login response as a session-token fallback, while returning only bearer-capable session state to host apps.
