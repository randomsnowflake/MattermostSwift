...
- Thread read state: `PUT /api/v4/users/{user_id}/teams/{team_id}/threads/{thread_id}/read/{timestamp}`, where `timestamp` is a Mattermost millisecond server timestamp such as a post `create_at` or thread `last_reply_at`.
...
- WebSocket event delivery: Mattermost documents the `/api/v4/websocket` authentication challenge and the event list, including `channel_viewed`, `multiple_channels_viewed`, `post_unread`, `response`, `thread_updated`, `thread_follow_changed`, and `thread_read_changed`; the SDK treats these as typed invalidation/thread signals where practical.
...
Personal access tokens are treated as bearer/session tokens for REST authentication. Username/password login sends `X-Requested-With: XMLHttpRequest`, prefers the `Token` response header, and accepts an `MMAUTHTOKEN` cookie from that same login response. The token is returned to the host as bearer-capable session state. The default SDK session is ephemeral and disables cookie persistence/handling; it does not use a host browser cookie jar or store credentials.
